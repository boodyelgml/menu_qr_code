   ...
   <div class="collapse " id="collapseExample" style="">
      <div class="card">
         ...
         <div class="card-header font-bold">
            <h5>Add New popup Ads</h5>
         </div>

         <div class="card-body ">
            <form class="form-horizontal" id="addTopAdForm" action="{{route('createPopUpAds')}}" method="POST" enctype="multipart/form-data">
               @csrf
               <div class="row">
...
<div class="col-12">
   <div class="card" id="adsDiv">
      ...
      <div class="card-header font-bold">
         <h5>Popup Ads List</h5>
      </div>

      <div class="card-body">
         @if(count($popupAds) < 1 )
         <div class="alert alert-danger font-bold font-16">Oops , it seems there is no ads
            ...
            <tbody>
               ...
               <tr>
                  <td>
                     ...
                     {{-- Ad photo view --}}
                     @if (strlen($popUpAd->photo) > 2 )

                     {{-- if photo exists --}}
                     <img data-toggle="modal" data-target=".bd-example-modal-lg" src="{{asset('assets/images/popUpAdPhoto').'/'.$popUpAd->photo}}" alt="Ad" height="60px" class="hvr-grow">
                     @else

                     {{-- photo placeholder --}}
                     <img data-toggle="modal" data-target=".bd-example-modal-lg" src="{{asset('assets/images/popUpAdPhoto/placeholder.jpg')}}" class="hvr-grow" alt="..." height="60px">
                     @endif
                  </td>

                  {{-- ad description --}}
                  <td> {{$popUpAd->description}} </td>

                  {{-- ad link --}}
                  <td> {{$popUpAd->ad_link}} </td>

                  {{-- ad video url --}}
                  <td> {{$popUpAd->video_url}} </td>
                  ...
                  {{-- restaurant --}}
                  <td> <a href="{{ route('showRestaurant' , $popUpAd->restaurant->id) }}">
                     {{$popUpAd->restaurant->name}} </a>
                  </td>

                  {{-- created at --}}
                  <td>{{$popUpAd->created_at->format('d F Y, h:i A')}} </td>

                  {{-- Actions --}}
                  <td>
                      <a href="{{route('editPopUpAds' , $popUpAd->id)}}" class="btn btn-primary"><i class="dripicons-gear"></i></a>
                      <button id="deleteTopAdButton" restaurant_id="{{route('deletePopUpAds' , $popUpAd->id)}}" class="btn btn-icon waves-effect waves-light btn-danger" type="submit"><i class="dripicons-trash"></i></button>
                  </td>
               </tr>
               @endforeach
            </tbody>
